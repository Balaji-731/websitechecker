...
class DataValidation:
    ...
    def validate_number_of_columns(self,dataframe:pd.DataFrame)->bool:
        try:
            number_of_columns=len(self._schema_config["columns"].keys())
    ...
    def has_numerical_columns(self, dataframe: pd.DataFrame) -> bool:
        try:
            ...
            logging.info(f"Number of numerical columns: {len(numerical_columns)}")
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            if not train_status:
                error_message += "Train data frame does not contain all required columns.\n"

            if not test_status:
                error_message += "Test data frame does not contain all required columns.\n"


            ## validate numerical columns
            # Validate train and test numerical columns
            if not self.has_numerical_columns(train_dataframe):
                error_message += "Train data frame does not contain numerical columns.\n"

            if not self.has_numerical_columns(test_dataframe):
                error_message += "Test data frame does not contain numerical columns.\n"
